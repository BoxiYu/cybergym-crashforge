...
SERVER_LOG_DIR="${SPLIT_GROUP_SERVER_LOG_DIR:-$ROOT/server_poc_${GROUP_STEM}_binary}"
...
prepare_wave() {
  log "preparing retry manifests for $GROUP_FILE -> $WAVE_DIR"
  "$PYTHON_BIN" "$ROOT/scripts/prepare_split_group_retry_wave.py" \
...
start_queue_launcher() {
  ...
  log "starting retry queue launcher for $WAVE_NAME"
  : >"$LAUNCHER_LOG"
...
main() {
  ...
  prepare_wave
  start_queue_launcher
  log "split-group retry wave staged: $WAVE_NAME"
}
