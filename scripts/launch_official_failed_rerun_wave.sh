...
SOURCE_DIR="${1:-$ROOT/reports/official_level1_failed_rerun_2026-07-29_d1_source}"
WAVE_NAME="${2:-official_level1_failed_rerun_2026-07-29_d1}"

mkdir -p "$SOURCE_DIR"

python3 "$ROOT/scripts/build_official_failed_rerun_wave.py" \
  --scoreboard-json "$ROOT/reports/official_level1_scoreboard_2026-07-28.json" \
  --tasks-json "$ROOT/cybergym_data/tasks.json" \
  --output-dir "$SOURCE_DIR" \
  --count "${OFFICIAL_FAILED_RERUN_COUNT:-24}" \
  --require-clean-local-only \
  --require-evidence-complete \
  --exclude-forbidden-access \
  --exclude-network-flagged

if [[ ! -s "$SOURCE_DIR/tasks.md" ]]; then
  echo "No official failed-rerun tasks were selected." >&2
  exit 1
fi

export SPLIT_GROUP_POLICY_PROFILE="${SPLIT_GROUP_POLICY_PROFILE:-official_level1}"
export SPLIT_GROUP_BINARY_PORT="${SPLIT_GROUP_BINARY_PORT:-18831}"
...
export SPLIT_GROUP_MAX_ACTIVE="${SPLIT_GROUP_MAX_ACTIVE:-8}"
